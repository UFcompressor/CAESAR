...
class Compressor(nn.Module):
    ...
    def decompress_for_cpp(self, q_latent, device='cuda'):
        return self.decode(q_latent.to(device))
...
class CompressorMix(nn.Module):
    ...
    def decompress_hyper(self, x):
        mean, scale = self.entropy_model.decompress_hyper_for_cpp(x, self.device)
        return mean, scale

    def decompress(self, x):
        
        outputs = self.entropy_model.decompress_for_cpp(x, self.device)
