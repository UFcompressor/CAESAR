...
// Deterministic block grid over (B,C,T,H,W) given block_size (bt,bh,bw).
// Called identically from compress() and decompress() so that block i in
// `blocks` always corresponds to slice i here -- this is what lets us drop
// the per-block coordinate fields entirely.
static std::vector<SliceDesc> make_slices(const Shape5& S,
...
// NOTE: b,c,t0..w1 are still passed in (needed to index into x_ptr/x0n_ptr/
// r_ptr), but are no longer written into the returned LBRCBlock -- they're
// regenerated by make_slices() at decode time instead of being serialized.
...
void compress(const torch::Tensor& original,
  ...
  // hard code it on the CPU
    ...
    // ---- fill metadata ----
    // shape/encoded_nrmse intentionally not stored: shape is recoverable from
    // the recons tensor at decode time, and encoded_nrmse was never the true
    // achieved value and was never read anywhere.
    meta.x_mean  = x_mean;
    meta.scale   = scale;
...
torch::Tensor decompress(const torch::Tensor&          recons,
    ...
    // Regenerate the exact same block grid used at encode time. blocks[i]
    // always corresponds to slices[i] because both compress() and
    // decompress() build the grid the same way from (S, meta.block_size).
    const auto slices = make_slices(S, meta.block_size);
    TORCH_CHECK(slices.size() == blocks.size(),
                ...
                "); recons shape or meta.block_size does not match encode time");
