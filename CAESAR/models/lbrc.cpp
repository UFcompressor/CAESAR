...
static std::vector<SliceDesc> make_slices(
    const Shape5& S, const std::array<int64_t, 3>& block_size) {
...
static void pack_bits(const std::vector<uint32_t>& v, uint32_t bit,
  ...
  for (size_t i = 0; i < v.size(); ++i)
    if ((v[i] >> bit) & 1u) out[i / 8] |= static_cast<uint8_t>(1u << (i % 8));
}

static void unpack_bits(const std::vector<uint8_t>& p, uint32_t bit,
                        std::vector<uint32_t>& out) {
  for (size_t i = 0; i < out.size(); ++i)
    if ((p[i / 8] >> (i % 8)) & 1u) out[i] |= 1u << bit;
}
...
static std::vector<int32_t> inv_lorenzo_3d(std::vector<int32_t> q, int64_t T,
  ...
  for (int64_t t = 0; t < T; ++t)
    for (int64_t h = 0; h < H; ++h)
      for (int64_t w = 1; w < W; ++w) q[idx(t, h, w)] += q[idx(t, h, w - 1)];
  for (int64_t t = 0; t < T; ++t)
    for (int64_t h = 1; h < H; ++h)
      for (int64_t w = 0; w < W; ++w) q[idx(t, h, w)] += q[idx(t, h - 1, w)];
  for (int64_t t = 1; t < T; ++t)
    for (int64_t h = 0; h < H; ++h)
      for (int64_t w = 0; w < W; ++w) q[idx(t, h, w)] += q[idx(t - 1, h, w)];
  return q;
}
...
static QuantResult quantize_block(const float* x, const float* x0n,
  ...
  if (sse0 <= target_sse) return {1.0, std::vector<int32_t>(N, 0), sse0};
...
static void compress_cpu(const torch::Tensor& original,
                         ...
                         int workers) {
  TORCH_CHECK(
      original.dtype() == torch::kFloat32 && recons.dtype() == torch::kFloat32,
...
static torch::Tensor decompress_cpu(const torch::Tensor& recons,
  ...
  for (int64_t i = 0; i < N_total; ++i) x0n[i] = (x0_ptr[i] - x_mean) / scale;
...
static void compress_gpu(const torch::Tensor& original,
  ...
  blocks.resize(Nb);
  for (auto& blk : blocks) blk.streams.clear();
...
static torch::Tensor decompress_gpu(const torch::Tensor& recons,
  ...
  uint32_t max_bits = 0;
  for (auto& b : blocks) max_bits = std::max(max_bits, b.bit_count);
  ...
  for (uint32_t bit = 0; bit < max_bits; ++bit) {
    ...
    for (int64_t i = 0; i < Nb; ++i)
      if (bit < blocks[i].bit_count) plane_bytes[i] = blocks[i].streams[bit];
  ...
  for (int64_t i = 0; i < Nb; ++i) steps[i] = blocks[i].step;
