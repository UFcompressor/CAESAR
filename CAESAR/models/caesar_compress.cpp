...
torch::Tensor Compressor::deblockHW(const torch::Tensor &data, int64_t nH,
  ...
  return merged.index({torch::indexing::Slice(), torch::indexing::Slice(),
                       torch::indexing::Slice(),
...
torch::Tensor unpadding(const torch::Tensor &padded_data,
  ...
  return padded_data.index({torch::indexing::Ellipsis,
...
CompressionResult Compressor::compress(const DatasetConfig &config,
  ...
  for (size_t i = 0; i < dataset.size(); i++) {
    ...
    if (cur_count == static_cast<int64_t>(batch_size) ||
      ...
      torch::Tensor norm_output =
          reshape_batch_2d_3d(raw_output, num_input_samples);
  ...
  if (result.use_lbrc) {
    ...
    torch::Tensor recon_ =
        recon_deblk.to(device_).to(torch::kFloat32).contiguous();
  ...
  torch::Tensor original_for_stats =
      (pad_T > 0) ? original_full.index(
