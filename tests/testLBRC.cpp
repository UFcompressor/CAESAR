...
static void test_zigzag() {
  const std::vector<int32_t> vals = {
      0, 1, -1, 127, -128, 32767, -32768, INT32_MAX / 2, INT32_MIN / 2};
  for (int32_t v : vals) {
    uint32_t u = zzenc(v);
    int32_t r = zzdec(u);
    assert(r == v);
...
static std::vector<int32_t> _lorenzo(const std::vector<int32_t> &q, int64_t T,
                                     int64_t H, int64_t W) {
...
static std::vector<int32_t> _inv_lorenzo(std::vector<int32_t> q, int64_t T,
                                         int64_t H, int64_t W) {
  auto ix = [&](int64_t t, int64_t h, int64_t w) {
    return (int64_t)((t * H + h) * W + w);
  };
