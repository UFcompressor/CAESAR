...
double computeNRMSE(const torch::Tensor &original,
...
std::tuple<torch::Tensor, double, double, double>
normalizeData(const torch::Tensor &data) {
...
torch::Tensor denormalizeData(const torch::Tensor &data, double x_min,
                              double x_max, double x_mean) {
...
void testPCACompressor2() {
  ...
  torch::Tensor original_data =
      torch::randn({num_vectors, vector_size}, torch::kFloat32);

  // Generate reconstructed data (with some error to simulate NN reconstruction)
  torch::Tensor recons_data =
      original_data +
      torch::randn({num_vectors, vector_size}, torch::kFloat32) * 0.1;

  // Get data statistics
  int64_t data_size_bits =
      original_data.numel() * original_data.element_size() * 8;
  double data_size_gbytes = original_data.nbytes() / (1024.0 * 1024.0 * 1024.0);

  std::cout << "Original/Recons Shape: [" << num_vectors << ", " << vector_size
            << "]" << std::endl;
  std::cout << "Original Data Size: " << data_size_bits << " bits" << std::endl;
  std::cout << "Bits Per Element: " << original_data.element_size() * 8
            << std::endl;
  ...
  std::cout << "Data range: [" << x_min << ", " << x_max
            << "], mean: " << x_mean << std::endl;
  ...
  std::cout << "Original CR from NN: "
            << static_cast<double>(data_size_bits) / latent_bits << "\n"
            << std::endl;
  ...
  for (double target_nrmse : test_nrmse_values) {
    std::cout << "\n--- Testing NRMSE Target: " << target_nrmse << " ---"
              << std::endl;

    auto start_time = std::chrono::high_resolution_clock::now();

    // Initialize compressor (matching Python: nrmse, 2, codec_algorithm="Zstd",
    // device=device)
    ...
    PCACompressor compressor(target_nrmse, quan_factor, device, codec_alg,
                             patch_size);
    ...
    if (compressed_bytes > 0) {
      recons_gae =
          compressor.decompress(recons_norm, compression_result.metaData,
                                *compression_result.compressedData);
    } else {
      std::cout << "No compression needed (all vectors within error bound)"
                << std::endl;
    ...
    double compression_ratio =
        static_cast<double>(original_data.nbytes()) / total_compressed_size;

    // Compute speeds
    auto encoding_time =
        std::chrono::duration<double>(encoding_end - start_time).count();
    auto decoding_time =
        std::chrono::duration<double>(decoding_end - encoding_end).count();
    auto total_time =
        std::chrono::duration<double>(decoding_end - start_time).count();
    ...
    std::cout << "  Compression Ratio: " << compression_ratio << "x"
              << std::endl;
    std::cout << "  Overall Speed:    " << overall_speed * 1024 << " MB/s"
              << std::endl;
    std::cout << "  Encoding Speed:   " << encoding_speed * 1024 << " MB/s"
              << std::endl;
    std::cout << "  Decoding Speed:   " << decoding_speed * 1024 << " MB/s"
              << std::endl;
    ...
    if (nrmse_error < target_nrmse * 1.1 || nrmse_error < 1e-6) {
      std::cout << "  Status:           ✓ PASSED (within tolerance)"
                << std::endl;
    } else {
      std::cout << "  Status:           ✗ FAILED (exceeds tolerance)"
                << std::endl;
    }
...
void testDecompression() {
  ...
  {
    std::cout << "\nTest 1: Full compress->decompress round-trip..."
              << std::endl;
    ...
    torch::Tensor originalData =
        torch::randn({numVectors, vectorSize}, torch::kFloat32);
    // Make sure reconstruction is different enough to trigger compression
    torch::Tensor reconsData =
        originalData + 1.5 * torch::randn_like(originalData);

    // Compress
    auto compressResult = compressor.compress(originalData, reconsData);

    if (compressResult.dataBytes == 0) {
      std::cout << "Warning: No compression occurred, skipping test"
                << std::endl;
      return;
    }

    std::cout << "Compressed data bytes: " << compressResult.dataBytes
              << std::endl;

    // Decompress
    torch::Tensor decompressed = compressor.decompress(
        reconsData, compressResult.metaData, *compressResult.compressedData);
    ...
    std::cout << "Max norm: " << maxNorm << ", Error bound: " << errorBound
              << std::endl;
    assert(maxNorm <=
           errorBound * 1.01); // Allow 1% tolerance for numerical precision
  ...
  // Test 2: Block format round-trip
  {
    ...
    torch::Tensor reconsData =
        originalData + 2.0 * torch::randn_like(originalData);
  ...
  // Test 3: Different patch sizes
  {
    ...
    torch::Tensor originalData =
        torch::randn({numVectors, vectorSize}, torch::kFloat32);
    torch::Tensor reconsData =
        originalData + 1.0 * torch::randn_like(originalData);
    ...
    if (compressResult.dataBytes > 0) {
      ...
      double nrmse_achieved =
          torch::norm(diff).item<double>() /
  ...
  // Test 4: Large dataset stress test
  {
    ...
    if (compressResult.dataBytes > 0) {
      ...
      auto compressTime = std::chrono::duration_cast<std::chrono::milliseconds>(
                              ...
                              .count();
      auto decompressTime =
          std::chrono::duration_cast<std::chrono::milliseconds>(endDecompress -
                                                                startDecompress)
              .count();

      std::cout << "Compression time: " << compressTime << " ms" << std::endl;
      std::cout << "Decompression time: " << decompressTime << " ms"
                << std::endl;

      double compressionRatio =
          (double)(numVectors * vectorSize * sizeof(float)) /
  ...
  {
    std::cout << "\nTest 5: Edge case - few unique coefficients..."
              << std::endl;
    ...
    if (compressResult.dataBytes > 0) {
      std::cout << "Unique values: "
                << compressResult.metaData.uniqueVals.size(0) << std::endl;
  ...
  // Test 7: Structured low-rank dataset
  {
    ...
    torch::Tensor reconsData =
        originalData + 0.1 * torch::randn_like(originalData);
  ...
  // Test 8: Very large dataset (~100MB)
  {
    ...
    std::cout << "Original size: " << (totalBytes / (1024.0 * 1024.0)) << " MB"
              << std::endl;
    ...
    if (compressResult.dataBytes > 0) {
      ...
      double compressionRatio = (double)totalBytes / compressResult.dataBytes;
...
void testPCACompressor() {
  ...
  // Test 2: Simple compression with small data
  {
    ...
    for (int i = 0; i < numVectors; ++i) {
      originalData[i] =
          originalData[i] + 0.5 * originalData[0]; // Add correlation
    }

    // Reconstruction data (slightly different)
    torch::Tensor reconsData =
        originalData + 0.05 * torch::randn_like(originalData);
    ...
    try {
      ...
      // FIX: Check if data was actually compressed before accessing tensor
      // dimensions
      if (result.dataBytes > 0 && result.metaData.pcaBasis.numel() > 0) {
        std::cout << "PCA basis shape: [" << result.metaData.pcaBasis.size(0)
                  << ", " << result.metaData.pcaBasis.size(1) << "]"
                  << std::endl;
        std::cout << "Number of vectors processed: " << result.metaData.nVec
                  << std::endl;
        std::cout << "Unique values count: "
                  << result.metaData.uniqueVals.size(0) << std::endl;
      } else {
        std::cout << "No compression needed - all residuals within error bounds"
                  << std::endl;
  ...
  // Test 3: Edge case - no data to compress
  {
    ...
    torch::Tensor reconsData =
        originalData + 0.001 * torch::randn_like(originalData);
    ...
    assert(result.compressedData != nullptr &&
           "compressedData should never be null");
    assert(result.compressedData->dataBytes == 0 &&
           "dataBytes should be 0 for empty compression");
    assert(result.compressedData->data.empty() &&
           "data vector should be empty");
    assert(result.metaData.pcaBasis.size(0) == 0 &&
           "pcaBasis should have 0 rows");
  ...
  // Test 5: Performance test with larger data
  {
    ...
    torch::Tensor originalData =
        torch::randn({numVectors, vectorSize}, torch::kFloat32);
    torch::Tensor reconsData =
        originalData + 0.02 * torch::randn_like(originalData);
    ...
    try {
      ...
      std::cout << "Large dataset compression completed in " << duration
                << " ms" << std::endl;
      std::cout << "Data bytes: " << result.dataBytes << std::endl;
      ...
      if (result.dataBytes > 0) {
        std::cout << "Compression ratio: "
                  << (static_cast<double>(numVectors * vectorSize *
                                          sizeof(float)) /
                      result.dataBytes)
                  << ":1" << std::endl;
      } else {
        std::cout << "No compression needed - all data within error bounds"
                  << std::endl;
      }
  ...
  // Test 6: Block data format (3D/4D tensors)
  {
    ...
    try {
      ...
      std::cout
          << "Note: Test 6 may fail if block2Vector is not fully implemented"
          << std::endl;
  ...
  {
    std::cout << "Test 7: High reconstruction error (force float64 PCA)..."
              << std::endl;
  ...
  // Test 8: Very small dataset (single vector)
  {
    ...
    std::cout << "Data bytes: " << result.dataBytes << std::endl;
    std::cout << "Unique values count: " << result.metaData.uniqueVals.size(0)
              << std::endl;
    std::cout << "Test 8 passed" << std::endl;
  }
...
int main() {
  ...
  std::cout << "Testing block2Vector <-> vector2Block round-trip..."
            << std::endl;
  ...
  std::vector<int64_t> originalShape(original.sizes().begin(),
                                     original.sizes().end());
  ...
  auto arr_2d = torch::tensor({{0, 1, 0, 0}, {1, 0, 0, 1}, {0, 0, 0, 1}},
                              torch::kFloat32);
  ...
  // Step 2: run reverse
  auto arr_2d_reconstructed =
      indexMaskReverse(prefix_mask, mask_length, arr_2d.size(1));
