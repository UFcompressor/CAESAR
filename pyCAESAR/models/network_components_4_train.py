"""Network components used only by the trainable CAESAR model.

NOTE: These components are kept separate from ``network_components.py``, which
belongs to the AOTI inference/compilation path.
"""
...
class FlexiblePrior(nn.Module):
    ...
    def __init__(self, channels=256, dims=[3, 3, 3], init_scale=10.0):
