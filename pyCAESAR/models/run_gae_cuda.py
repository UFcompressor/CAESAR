...
def block2vector(block_data, patch_size=(8, 8)):
    ...
    *leading_dims, T, H, W = block_data.shape
    ...
    vector_data = (
        block_data.view(*leading_dims, T, n_h, patch_size[0], n_w, patch_size[1])
...
def vector2block(vectors, original_shape, patch_size=(8, 8)):
    *leading_dims, T, H, W = original_shape
    ...
    block_data = (
        vectors.view(*leading_dims, n_h, n_w, T, patch_size[0], patch_size[1])
...
def index_mask_prefix(arr_2d: torch.Tensor):
    ...
    mask = torch.arange(num_cols, device=arr_2d.device).unsqueeze(
        0
...
def index_mask_reverse(prefix_mask, mask_length, num_cols):
    ...
    mask = torch.arange(num_cols, device=device).unsqueeze(0) <= mask_length.unsqueeze(
        1
    )
...
class PCACompressor:
    ...
    def compress(self, original_data, recons_data):
        ...
        quan_coeff_sorted = (
            torch.round(all_coeff_sorted / self.quan_bin) * self.quan_bin
        )
        ...
        all_coeff_power_desc = torch.gather(
            all_coeff_power, 1, sort_index
        ) - res_coeff_sorted.pow(2)
        ...
        coeff_int_flatten = torch.round(
            all_coeff.reshape([-1])[mask.reshape(-1)] / self.quan_bin
        )
        unique_vals, coeff_int_flatten = torch.unique(
            coeff_int_flatten, return_inverse=True
        )
    ...
    def compress_lossless(self, meta_data, main_data):
        ...
        process_mask_uint8 = np.packbits(
            main_data["process_mask"].cpu().numpy().astype(np.uint8)
        )
        prefix_mask_uint8 = np.packbits(
            main_data["prefix_mask"].cpu().numpy().astype(np.uint8)
        )

        data_bytes = (
            meta_data["pca_basis"].cpu().numpy().nbytes
            + len(meta_data["unique_vals"]) * 4
        )
        ...
        if self.device == "cpu":
            ...
            compressed_data = {
                ...
                "mask_length": cctx.compress(
                    main_data["mask_length"].cpu().numpy().tobytes()
                ),
                "coeff_int": cctx.compress(
                    main_data["coeff_int"].cpu().numpy().tobytes()
                ),
            }
    ...
    def decompress_lossless(self, meta_data, compressed_data):
        ...
        process_mask = np.asarray(
            codec.decode(compressed_data["process_mask"], "|u1").cpu()
        )

        process_mask = bytes_to_bits(process_mask, meta_data["n_vec"])
        process_mask = torch.from_numpy(np.ascontiguousarray(process_mask)).to(
            self.device, non_blocking=True
        )

        prefix_mask = np.asarray(
            codec.decode(compressed_data["prefix_mask"], "|u1").cpu()
        )
        prefix_mask = bytes_to_bits(prefix_mask, meta_data["prefix_length"])
        prefix_mask = torch.from_numpy(np.ascontiguousarray(prefix_mask)).to(
            self.device, non_blocking=True
        )

        mask_length = torch.from_numpy(
            np.asarray(codec.decode(compressed_data["mask_length"], "|u1").cpu())
        ).to(self.device, non_blocking=True)

        coeff_int = torch.from_numpy(
            np.asarray(
                codec.decode(
                    compressed_data["coeff_int"], meta_data["coeff_dtype"]
                ).cpu()
            )
        ).to(self.device, non_blocking=True)
    ...
    def decompress_lossless_cpu(self, meta_data, compressed_data):
        ...
        process_mask = np.frombuffer(
            dctx.decompress(compressed_data["process_mask"]), dtype=np.uint8
        )
        process_mask = bytes_to_bits(process_mask, meta_data["n_vec"])

        prefix_mask = np.frombuffer(
            dctx.decompress(compressed_data["prefix_mask"]), dtype=np.uint8
        )
        prefix_mask = bytes_to_bits(prefix_mask, meta_data["prefix_length"])

        mask_length = np.frombuffer(
            dctx.decompress(compressed_data["mask_length"]), dtype=np.uint8
        )
        dtype_map = {"|u1": np.uint8, "<i2": np.int16, "<i4": np.int32}
        coeff_int = np.frombuffer(
            dctx.decompress(compressed_data["coeff_int"]),
            dtype=dtype_map[meta_data["coeff_dtype"]],
        )
        print(
            "dtype=dtype_map[meta_data[coeff_dtype]]",
            dtype_map[meta_data["coeff_dtype"]],
        )
    ...
    def decompress(self, recons_data, meta_data, compressed_data, to_np=True):
        ...
        index_mask = index_mask_reverse(
            main_data["prefix_mask"],
            main_data["mask_length"],
            meta_data["pca_basis"].shape[0],
        )
        ...
        recons_data[process_mask] = (
            recons_data[process_mask] + (coeff @ pca_basis).float()
        )
...
def run_gae(args):
    ...
    print(
        ...
        "Latent Bits:",
        args.latent_bit,
        "Original CR: %.2f" % (data_size_bit / args.latent_bit),
        original_data.dtype,
    )
    ...
    for nrmse in all_nrmse:
        ...
        save_path = (
            args.path.replace(".npz", "") + "_nrmse_%.6f.npz" % (nrmse)
            if args.save_path == ""
            else args.save_path
        )
        ...
        for alg in all_algs:  # ,  "Cascaded"
            ...
            compressor = PCACompressor(
                nrmse, quan_factor, codec_algorithm=alg, device=device
            )
            meta_data, compressed_data, data_bytes = compressor.compress(
            ...
            if data_bytes > 0:
                recons_data_gae = compressor.decompress(
                    recons_data, meta_data, compressed_data
                )
            else:
            ...
            print(
                alg,
                "Targe NRMSE: %.6f" % nrmse,
                "Final NRMSE: %.6f" % nrmse_final,
                "CR: %.1f" % (CR),
                "Processing Speed:  %.3f GB/s (%.3f || %.3f)"
                % (speed_overall, encoding_speed, decoding_speed),
            )
        ...
        if args.save_result:
            if args.save_original:
                np.savez(
                    save_path,
                    original_data=original_data * (x_max - x_min) + x_mean,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--nrmse", type=str, default="0.001,0.004,0.002,0.001,0.0005,0.0002,0.0001"
    )
    parser.add_argument(
        "--path", type=str, default="/blue/ranka/shared-xiaoli/example.npz"
    )
