...
class CAESAR:
    ...
    def decompress_caesar_d(self, all_compressed, shape, filtered_blocks):
        ...
        with torch.no_grad():
            for compressed in all_compressed:
                ...
                rct_data = rct_data.reshape(
                    [B, -1, 16, *rct_data.shape[-2:]]
                ) * compressed["scale"].to(self.device) + compressed["offset"].to(
                    self.device
                )
    ...
    def decompress_caesar_v(self, all_compressed, shape, filtered_blocks):
        ...
        with torch.no_grad():
            for compressed in all_compressed:
                ...
                rct_data = self.compressor_v.decompress(
                    *compressed["compressed"], device=self.device
                )
                rct_data = rct_data * compressed["scale"].to(self.device) + compressed[
                    "offset"
                ].to(self.device)
                rct_data = rct_data.cpu()
    ...
    def postprocessing_encoding(self, original_data, recons_data, nrmse):
        ...
        self.compressor = PCACompressor(
            nrmse, 2, codec_algorithm="Zstd", device=self.pca_device
        )
